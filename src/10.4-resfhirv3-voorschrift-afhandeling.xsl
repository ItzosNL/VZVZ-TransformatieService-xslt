...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="transformationCode" as="xs:string" select="'10.4'"/>
	...
	<xsl:template match="/">
		...
		<xsl:variable name="targetMessageID">
			<xsl:choose>
				<xsl:when test="not($payload/hl7:PAMV_IN924000NL02)">
					<!-- dummy id -->
					<id extension="0012345678" root="2.16.840.1.113883.2.4.6.6.1.1" xsl:exclude-result-prefixes="#all"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:sequence select="$payload//hl7:PAMV_IN924000NL02/hl7:id"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
