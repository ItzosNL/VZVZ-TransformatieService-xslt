...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            <xd:p>Transformatie voor sturen voorstel verstrekkingsverzoek</xd:p>
            
            <xd:p><xd:b>Created on:</xd:b> 2023-08-04</xd:p>
    ...
    <xsl:variable name="transformationCode" as="xs:string" select="'11.1'"/>
    
    <xsl:variable name="buildingBlock" select="'PVVV'"/>
    <xsl:variable name="schematronRef" select="concat('file:', $svnAortaMP9, '/schematron_closed_warnings/mp-vzvz-verzendenVoorstelVerstrekkingsverzoek.sch')"/>
    <xsl:variable name="schematronRefZT" select="concat('file:', $svnAortaZTMP, '/mp-runtime-develop/mp-mp93_vvv.sch')"/>
    ...
    <xsl:template match="/">
        <xsl:variable name="payload">
            <xsl:choose>
                <xsl:when test="exists(./organizer/hl7:organizer)">
                    <xsl:for-each select="./organizer/hl7:organizer">
