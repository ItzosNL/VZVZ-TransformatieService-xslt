...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:f="http://hl7.org/fhir"
  xmlns:hl7="urn:hl7-org:v3" xmlns:hl7nl="urn:hl7-nl:v3" xmlns:vf="http://www.vzvz.nl/functions"
  xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" xmlns:saxon="http://saxon.sf.net/"
  xmlns:util="urn:hl7:utilities" xmlns:math="http://www.w3.org/2005/xpath-functions/math"
  exclude-result-prefixes="xs xsi vf s xd hl7 hl7nl math saxon util f" version="3.0"
  ...
  <!-- 2025-01-21 remove original query param because the necessary ids are part of the metadata, 
        so this file becomes backwards incompatible with the previous version
    -->
    ...
    <authorOrPerformer xmlns="urn:hl7-org:v3" typeCode="AUT">
      <participant>
        <xsl:choose>
          <xsl:when test="
              $author/ID[
              contains(., '2.16.528.1.1007.3.1') or
              contains(., '2.16.840.1.113883.2.4.6.3')]">
  ...
  <xsl:template name="buildOverseer">
    ...
    <overseer xmlns="urn:hl7-org:v3" typeCode="RESP">
      <xsl:choose>
        <xsl:when test="
            contains($overseer/ID, '2.16.528.1.1007.3.1') or
            contains($overseer/ID, '2.16.840.1.113883.2.4.6.3')">
          <xsl:call-template name="buildAssignedPerson">
            <xsl:with-param name="person" select="$overseer"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:when test="
            contains($overseer/ID, '2.16.528.1.1007.3.2') or
            contains($overseer/ID, '2.16.528.1.1007.3.3')">
          <xsl:call-template name="buildAssignedDevice">
      ...
      <xsl:choose>
        <xsl:when test="exists($person/AssignedPerson)">
          <assignedPrincipalChoiceList>
            <assignedPerson>
              <name>
                <xsl:value-of select="$person/AssignedPerson/Name"/>
              </name>
            </assignedPerson>
          </assignedPrincipalChoiceList>
        </xsl:when>
      ...
      <Organization>
        ...
        <xsl:choose>
          <xsl:when test="exists($person/Org/Name)">
            <name>
              <xsl:value-of select="$person/Org/Name"/>
            </name>
          </xsl:when>
          <xsl:otherwise>
            <name>N/A</name>
        ...
        <xsl:choose>
          <xsl:when test="exists($person/Org/Place)">
            <addr>
              <city>
                <xsl:value-of select="$person/Org/Place"/>
              </city>
            </addr>
          </xsl:when>
          <xsl:otherwise>
  ...
  <xd:doc>
    ...
    <xd:param name="elementName">name of the surrounding element, defaults to
      AssignedDevice</xd:param>
  </xd:doc>
      ...
      <Organization xmlns="urn:hl7-org:v3">
        ...
        <xsl:choose>
          <xsl:when test="exists($device/Org/Name)">
            <name>
              <xsl:value-of select="$device/Org/Name"/>
            </name>
          </xsl:when>
          <xsl:otherwise>
            <name>N/A</name>
        ...
        <xsl:choose>
          <xsl:when test="exists($device/Org/Place)">
            <addr>
              <city>
                <xsl:value-of select="$device/Org/Place"/>
              </city>
            </addr>
          </xsl:when>
          <xsl:otherwise>
  ...
  <xd:doc>
    ...
    <xd:param name="payload">the original payload that is already fixed with the correct
      template</xd:param>
    <xd:param name="transformationCode">transformationCode of the current transformation</xd:param>
...
-->
          ...
          <xsl:variable name="queryMessage">
            ...
            <xsl:choose>
              <xsl:when
                test="exists($metaDataObj/Meta/OrigMessageId) and exists($metaDataObj/Meta/OrigQueryId)">
                <xsl:choose>
              ...
              <!--                            <xsl:when test="exists($originalQueryObj/hl7:id)">
                                <xsl:variable name="firstChild" select="$originalQueryObj/*[local-name() = $queryInteractionID]"/>
                                ...
                                <xsl:sequence select="$firstChild//hl7:ControlActProcess/hl7:queryByParameter/hl7:queryId" />
                            </xsl:when>
-->
              <xsl:otherwise>
                <!-- dummy id -->
    ...
    <interactionId xmlns="urn:hl7-org:v3" extension="{$interactionID}" root="2.16.840.1.113883.1.6"/>
    ...
    <ControlActProcess xmlns="urn:hl7-org:v3" moodCode="EVN">
      ...
      <xsl:for-each select="$payload/hl7:organizer | $payload/hl7:ClinicalDocument">
        <subject>
          <xsl:choose>
            <xsl:when
              test="exists($payload/hl7:organizer) and not(exists($payload/hl7:organizer/hl7:recordTarget))">
              <organizer>
                <xsl:copy-of select="@*"/>
        ...
        <acknowledgement typeCode="CE">
          <acknowledgementDetail>
            <code xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" code="SYN105"
              codeSystem="2.16.840.1.113883.5.1100" displayName="Required element missing"
              xsi:type="CV"/>
            <text>
              <xsl:value-of
  ...
  <xd:doc>
    ...
    <xd:param name="transformationCode">The transformation code of this transformation</xd:param>
  ...
  <xd:doc>
    ...
    <xd:param name="schematronRefZT">schematron files of the Zorgtoepassing to be
      included</xd:param>
    <xd:param name="schemaRefDir">directory of the XML schema definition to be included</xd:param>
  ...
  <xsl:template name="buildV3message">
    ...
    <xsl:if test="$xslDebug">
      <xsl:processing-instruction name="xml-model">phase="#ALL" href="<xsl:value-of select="$schematronRef"/>
" type="application/xml" schematypens="http://purl.oclc.org/dsdl/schematron"</xsl:processing-instruction>
      <xsl:if test="string-length($schematronRefZT) &gt; 0">
        <xsl:processing-instruction name="xml-model">phase="#ALL" href="<xsl:value-of select="$schematronRefZT"/>
" type="application/xml" schematypens="http://purl.oclc.org/dsdl/schematron"</xsl:processing-instruction>
      </xsl:if>
    </xsl:if>
    ...
    <xsl:element name="{$interactionId}" namespace="urn:hl7-org:v3">
      <xsl:if test="$xslDebug">
        ...
        <xsl:attribute name="xsi:schemaLocation">urn:hl7-org:v3 <xsl:value-of select="$schemaRef"/>
        </xsl:attribute>
      </xsl:if>
