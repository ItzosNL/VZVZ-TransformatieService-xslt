...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            <xd:p>Transformatie voor sturen afhandelen voorstel medicatieafspraak</xd:p>
    ...
    <xsl:variable name="transformationCode" as="xs:string" select="'13.1'"/>
    
    <xsl:variable name="buildingBlock" select="'PAVM'"/>
    <xsl:variable name="schematronRef" select="concat('file:', $svnAortaMP9, '/schematron_closed_warnings/mp-vzvz-verzendenAntwoordVoorstelMedicatieafspraak.sch')"/>
    <xsl:variable name="schematronRefZT" select="concat('file:', $svnAortaZTMP, '/mp-runtime-develop/mp-mp93_avma.sch')"/>
