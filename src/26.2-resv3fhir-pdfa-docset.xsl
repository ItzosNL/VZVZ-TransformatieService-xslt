...
<xsl:stylesheet xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" exclude-result-prefixes="#all"
    ...
    extension-element-prefixes="func vf hl7 math">
    ...
    <xsl:variable name="transformationCode" as="xs:string" select="'26.2'"/>
                    ...
                    <xsl:if
                        ...
                        <entry>
                            <fullUrl>
                                <xsl:attribute name="value">
                                    <xsl:value-of
                                        select="concat('urn:uuid:', substring($DocID, 1, 33), substring(string(100 + position()), 1, 3))"
                                    />
                                </xsl:attribute>
                            </fullUrl>
                            <resource>
                                <xsl:call-template name="DocMan_translate">
                        ...
                        <entry>
                            <fullUrl>
                                <xsl:attribute name="value">
                                    <xsl:value-of
                                        select="concat('urn:uuid:', substring($DocID, 1, 33), substring(string(200 + position()), 1, 3))"
                                    />
                                </xsl:attribute>
                            </fullUrl>
                            <resource>
                                <xsl:call-template name="DocRef_translate">
                                    <xsl:with-param name="Base"
                                        select="substring-before($bundleSelfLink, '/DocumentManifest')"/>
                                    <xsl:with-param name="AppID"
                                        select="../../../../hl7:sender/hl7:device/hl7:id/@extension"/>
                                    <xsl:with-param name="DocID" select="$DocID"/>
                                    <xsl:with-param name="DocNr" select="position()"/>
