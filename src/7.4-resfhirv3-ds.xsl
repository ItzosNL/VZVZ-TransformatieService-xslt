...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            <xd:p>Transformatie voor opleveren Doseerschema</xd:p>
...
<!--    <xsl:param name="originalQuery"/>-->
    ...
    <xsl:variable name="transformationCode" as="xs:string" select="'7.4'"/>
    
    <xsl:variable name="buildingBlock" select="'WDS'"/>
    <xsl:variable name="schematronRef" select="concat('file:', $svnAortaMP9, '/schematron_closed_warnings/mp-vzvz-opleverenDoseerschema.sch')"/>
    <xsl:variable name="schematronRefZT" select="concat('file:', $svnAortaZTMP, '/mp-runtime-develop/mp-mp93_mg_wds.sch')"/>
