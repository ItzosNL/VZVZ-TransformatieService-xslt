...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <!--
    ...
    -->
   <!-- ================================================================== -->
   <xsl:import href="2_fhir_fhir_include-d551e152.xsl"/>
   ...
   <xsl:variable name="ada2resourceType">
      ...
      <nm:map ada="anatomische_locatie"
              resource="BodyStructure"
              profile="nl-core-AnatomicalLocation-BodyStructure"/>
      <nm:map ada="meetlocatie"
              resource="BodyStructure"
              profile="nl-core-AnatomicalLocation-BodyStructure"/>
      <nm:map ada="probleem_anatomische_locatie"
              resource="BodyStructure"
              profile="nl-core-AnatomicalLocation-BodyStructure"/>
      ...
      <nm:map ada="behandeldoel"
              resource="Goal"
              profile="nl-core-TreatmentObjective"/>
      ...
      <nm:map ada="medisch_hulpmiddel"
              resource="DeviceUseStatement"
              profile="nl-core-AbilityToPerformMouthcareActivities.Prosthesis"/>
      <nm:map ada="medisch_hulpmiddel"
              resource="Device"
              profile="nl-core-AbilityToPerformMouthcareActivities.Prosthesis.Product"/>
      ...
      <nm:map ada="vermogen_tot_mondverzorging"
              resource="Observation"
              profile="nl-core-AbilityToPerformMouthcareActivities"/>
      ...
      <nm:map ada="visus_meet_hulpmiddel"
              resource="Device"
              profile="nl-core-VisualAcuity.VisualAcuityMeasurementDevice"/>
      ...
      <nm:map ada="woning_aanpassing"
              resource="Observation"
              profile="nl-core-LivingSituation.HomeAdaption"/>
      <nm:map ada="woon_omstandigheid"
              resource="Observation"
              profile="nl-core-LivingSituation.LivingCondition"/>
      <nm:map ada="woning_type"
              resource="Observation"
              profile="nl-core-LivingSituation.HouseType"/>
...
<xsl:value-of select="$groupKey"/>
   ...
   <xsl:template name="getLogicalIdFromFhirMetadata">
      ...
      <xsl:param name="profile"
                 as="xs:string"
                 select="''">
         <!-- The id of the profile that is being generated from the specified ADA instance. This is needed to specify which profile is targeted when a single ADA instance is mapped onto multiple FHIR profiles. It may be omitted otherwise. -->
      </xsl:param>
      <xsl:variable name="groupKey">
         <xsl:choose>
